...
describe('6-module-3-task', () => {
  ...
  it('MeetupForm должен выводить поля митапа с корректными значениями по умолчанию', () => {
    ...
    expect(new Date(inputs[1].value).toDateString()).toBe(
      meetup.date.toDateString(),
    );
  ...
  it('MeetupForm должен выводить формы редактирования программы митапа', async () => {
  ...
  it('MeetupForm должен порождать событие submit по сабмиту формы с новыми данными митапа', async () => {
    ...
    await inputs[1].setValue(stringDate);
